...
{	// IsEven(x)
	// IsEven(x) ::= IsEven(x - 2)
	//     x  
	// (1) A --> IsEven(x) ::= IF x > 1 THEN IsEven(x - 1 - 1) ELSE IsZero()
	// [1]:(A)	IsEven(x) ::= IF Greater(x, Succ(Zero()) THEN IsEven(Pred(Pred(x))) ELSE IsZero()
	return Greater(x, Succ(Zero())) ? IsEven(Pred(Pred(x))) : IsZero();
	// if (Greater(Succ(Succ(Zero())), x))		// if (2 > x)	x == (0 or 1)
	//	return IsZero(x);
	// return IsEven(Pred(Pred(x)));
}
